...
namespace CursedBroom.Aseprite {
    public sealed class AsepriteFile : ScriptableObject {
        ...
        public Texture2D emission => m_emission;
        ...
        public static bool TryCreateInstance(FileInfo asepriteFile, out AsepriteFile asset, AsepriteSettings settings, Action<Texture2D> texturePostProcessor = default) {
            ...
            asset.m_albedo = new(2, 2, TextureFormat.RGBA32, settings.textureMipChaining) {
                name = sheetFile.Name,
            ...
            asset.m_albedo.LoadImage(File.ReadAllBytes(sheetFile.FullName));

            texturePostProcessor?.Invoke(asset.m_albedo);
            ...
            if (settings.extractEmission) {
                ...
                if (emissionFile.Exists
                 ...
                 && emissionTexture.GetPixels().Any(color => !Mathf.Approximately(color.a, 0))) {
                    ...
                    Texture2D newEmissionTexture = new(asset.m_albedo.width, asset.m_albedo.height, TextureFormat.RGBA32, settings.textureMipChaining) {
                        name = emissionFile.Name,
                        filterMode = settings.textureFilterMode,
                        alphaIsTransparency = false,
                        anisoLevel = 0,
                    ...
                    newEmissionTexture.SetPixels32(new Color32[asset.m_albedo.width * asset.m_albedo.height]);
                    ...
                    asset.m_emission = newEmissionTexture;

                    texturePostProcessor?.Invoke(asset.m_emission);
